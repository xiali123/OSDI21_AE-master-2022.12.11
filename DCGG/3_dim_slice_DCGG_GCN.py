...
outfile = './log/3_dim_slice_GCN_' + mytime  +'.log'
...
outfile = './log/3_no_dim_slice_GCN_' + mytime +'.log'
is_dim_slice = False
for hid in hidden:
    for data, d, c, load, warp_size_k in dataset:
        loadFromTxt = load
        ...
        command = "{} GNNA_main.py --dataset {} --dim {} --hidden {} \
                    --classes {} --partSize {} --model {} --warpPerBlock {}\
                    --manual_mode {} --verbose_mode {} --enable_rabbit {} --loadFromTxt {}  --cur_dim_slice {} --max_warp_size {} --is_sort {} --is_dynamic_warp {} --is_data_stream {} --is_dim_slice {} | tee -a {}"
        command = command.format(python_cmd, data, d, hid, c, partsize, model, warpPerBlock, \
