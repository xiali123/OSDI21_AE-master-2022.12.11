...
outfile = './log/1_no_colum_slice_GCN_' +mytime +'.log'
...
column_slice_size_li = [1, 4, 8, 12, 16, 32, 48, 52, 60, 64, 96, 128]
    ...
    for hid in hidden:
        for data, d, c, load, warp_size_k in dataset:
            ...
            command = "{} GNNA_main.py --dataset {} --dim {} --hidden {} \
                        ...
                        --manual_mode {} --verbose_mode {} --enable_rabbit {} --loadFromTxt {} --warp_size_k {} --column_slice_size {} --max_warp_size {} --is_sort {} --is_dynamic_warp {} --is_data_stream {} --is_dim_slice {} | tee -a {}"
            command = command.format(python_cmd, data, d, hid, c, partsize, model, warpPerBlock, \
                                     manual_mode, verbose_mode, enable_rabbit, loadFromTxt, warp_size_k, column_slice_size, max_warp_size, is_sort, is_dynamic_warp, is_data_stream, is_dim_slice, outfile)
            # command = "python GNNA_main.py -loadFromTxt --dataset {} --partSize {} --dataDir {}".format(data, partsize, '/home/yuke/.graphs/orig')
            os.system(command)

outfile = './log/1_no_colum_slice_GCN_' + mytime +'.txt'
for hid in hidden:
    for data, d, c, load, warp_size_k in dataset:
        loadFromTxt = load
        outfile_new = outfile+"-"+data+".txt"
